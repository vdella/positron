...
class NeuralNetwork:
    ...
    def __init__(self, layers: list, learning_rate=0.01, hidden_act='sigmoid', output_act='sigmoid'):
        ...
        self.hidden_act = hidden_activation[hidden_act][0]
        self.hidden_act_derivative = hidden_activation[hidden_act][1]
    ...
    def forward(self, X):
        ...
        self.parameters[f'A1'] = self.hidden_act(self.parameters[f'Z1'])

        for l in range(2, len(self.layers)):
            self.parameters[f'Z{l}'] = (np.dot(
                self.parameters[f'A{l-1}'], self.parameters[f'W{l}'])
                                        + self.parameters[f'b{l}'])
            self.parameters[f'A{l}'] = self.hidden_act(self.parameters[f'Z{l}'])
        ...
        for l in range(1, len(self.layers))[::-1]:
            if l == len(self.layers) - 1:
                ...
                self.parameters[f'dZ{l}'] = (self.parameters[f'dA{l}']
                    * self.hidden_act_derivative(self.parameters[f'Z{l}']))
                self.parameters[f'dW{l}'] = np.dot(self.parameters[f'A{l - 1}'].T,
                                                   self.parameters[f'dZ{l}']) \
